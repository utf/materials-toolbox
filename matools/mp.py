...
def get_structures(mp, query, mp_structure=False, stable=False, stable_tol=0,
                   icsd_only=False):
    data = ['pretty_formula', 'e_above_hull', 'band_gap', 'nsites', 'volume',
            'icsd_id']
    stype = 'final' if mp_structure else 'initial'
    if ',' in query:
        elements = query.split(',')
        entries = mp.get_entries_in_chemsys(elements, inc_structure=stype,
                                            property_data=data)
    else:
...
def prompt_selection(entries, save_all=False):
    ...
    headers = ['n', 'Formula', 'Spacegroup', 'E above Hull', 'Band Gap',
               'Nsites', 'Volume', 'icsd_id']
    table = []
    for i, e in enumerate(entries):
        ...
        table.append([i+1, formula, spg, e_above_hull, band_gap, nsites, 
                      volume, icsd_id])
...
def main():
    ...
    parser.add_argument('query', help="""The material (e.g. ZnO), composition
    ...
    parser.add_argument('-s', '--stable', action='store_true',
                        ...
                        above hull is 0""")
    parser.add_argument('--stol', default=0., type=float,
    ...
    parser.add_argument('--cif', action='store_true',
                        ...
                        than as POSCARs""")
    parser.add_argument('--ase', default=None,
    ...
    parser.add_argument('--conv', action='store_true',
                        ...
                        primitive""")
    parser.add_argument('--mp_structure', action='store_true',
                        help="""Use the Materials Project relaxed structure
                        rather than the initial ICSD structure""")
    parser.add_argument('--icsd_only', action='store_true',
    ...
    parser.add_argument('--save_all', action='store_true',
                        help="""Download all structures found""")

    args = parser.parse_args()

    logging.basicConfig(filename='mp-get.log', level=logging.INFO,
                        filemode='w', format='%(message)s')
    console = logging.StreamHandler(stdout)
    logging.getLogger('').addHandler(console)

    mp = MPRester()
    entries = get_structures(mp, args.query, mp_structure=args.mp_structure,
                             stable=args.stable, icsd_only=args.icsd_only,
                             stable_tol=args.stol)
    ids = prompt_selection(entries, save_all=args.save_all)
    save_structures(entries, ids, cif=args.cif, conv=args.conv, ref=args.ref,
                    db=args.ase)
